class BonusAction < ActiveRecord::Base
  belongs_to :stock
  has_many :transactions, class_name: "BonusTransaction"
  has_many :bonus, through: :transactions, class_name: EquityBuy
  ...
  def apply_on_portfolio
    TradingAccount.all.each do |trading_account|
      Portfolio.all.each do |portfolio|
        ...
        buys = EquityBuy.where(stock_id: self.stock.id, trading_account_id: trading_account.id, portfolio_id: portfolio.id, delivery: true).where("date < '#{ex_date}'").to_a
        ...
        if bonus_qty > 0
          ...
          bonusable_buys.each do |buy|
            self.transactions << BonusTransaction.new(bonus: eq_transaction, source_transaction: buy)
          end
