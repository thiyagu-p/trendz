class MarketActivityController < ApplicationController
  ...
  def chart
    ...
    @market_activities = MarketActivity.find(:all, :conditions => "date > '#{start_date}' and date <= '#{max_date}'", :order => 'date asc')
    @nifty_quotes = nifty ? EqQuote.find(:all, :conditions => "date > '#{start_date}'  and date <= '#{max_date}' and stock_id = #{nifty.id}") : []
  end
end