...
describe Importer::Nse::CorporateActionImporter do
  ...
  context 'FT', ft: true do
    it 'import with full history', ft: true do
      stock = create(:stock, symbol: 'TCS', face_value: 10, nse_active: true)
  ...
  describe '.import' do
    ...
    context 'first time run' do
      it 'import one by one using more than 24 months data and future data' do
        reliance = create(:stock, symbol: 'RELIANCE', nse_active: true)
        lt = create(:stock, symbol: 'LT', nse_active: true)
        @http = stub()
    ...
    context 'parsing of data' do
      before :each do
        @stock1 = create(:stock, symbol: 'RELIANCE', nse_active: true)
        @http = stub()
        Net::HTTP.expects(:new).with(NSE_URL).returns(@http)
